...
from app.ai.agents.learning_objective_agent.Lo_refine_agent.prompts import (
    SYSTEM_PROMPT,
)
...
class LORefinementAgent:
    ...
    def run(self, input_data: LORefinementInput) -> LORefinementOutput:
        ...
        try:
            raw = chat(self._kernel, SYSTEM_PROMPT, user_msg, config, "LO_REFINE")
            data = json.loads(raw)
