...
from app.ai.agents.learning_objective_agent.config.llm import (
...
from app.ai.agents.learning_objective_agent.Lo_validator.prompts import (
    SYSTEM_PROMPT,
)
...
class LOValidatorAgent:
    ...
    def run(self, input_data: LOValidationInput) -> LOValidationOutput:
        ...
        try:
            raw = chat(self._kernel, SYSTEM_PROMPT, user_msg, config, "LO_VALIDATOR")
            data = json.loads(raw)
