"""HTTP routes for Required Topics generation and regeneration during onboarding."""
...
@router.post(
    ...
    response_model=RegenerateRequiredTopicsResponse,
    status_code=status.HTTP_200_OK,
)
async def regenerate_required_topics(
    payload: RegenerateRequiredTopicsRequest,
    kernel: Kernel = Depends(get_kernel),
) -> RegenerateRequiredTopicsResponse:
    """Revise existing required topics based on user feedback."""
    try:
        service = RequiredTopicService(kernel)
        return await service.regenerate_required_topics(payload)
    except Exception:
        logger.exception("Failed to regenerate required topics")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Could not regenerate required topics. Please try again.",
        )
