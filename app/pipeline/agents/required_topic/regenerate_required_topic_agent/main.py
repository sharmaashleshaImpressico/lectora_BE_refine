...
from app.pipeline.agents.required_topic.regenerate_required_topic_agent.config.llm import (
    make_config,
)
from app.pipeline.agents.required_topic.regenerate_required_topic_agent.models import (
    RTRegenerationInput,
...
def _execute(input_data: RTRegenerationInput) -> tuple[RTRegenerationOutput, str | None, bool]:
    ...
    try:
        raw = llm_chat(_SYSTEM_PROMPT, user_msg, config, "RT_REGEN")
        data = json.loads(raw)
...
class RTRegenerationAgent:
    ...
    def run(self, input_data: RTRegenerationInput) -> RTRegenerationOutput:
        ...
        try:
            _result, _error, _fell_back = _execute(input_data)
            return _result
        except Exception as exc:
            _error = str(exc)
            raise
