...
from app.pipeline.agents.required_topic.rt_validator.config.llm import (
    make_config,
)
...
from app.pipeline.agents.required_topic.rt_validator.prompts import (
    SYSTEM_PROMPT,
)
...
class RTValidatorAgent:
    ...
    def run(self, input_data: RTValidationInput) -> RTValidationOutput:
        ...
        try:
            try:
                raw = llm_chat(SYSTEM_PROMPT, user_msg, config, "RT_VALIDATOR")
                data = json.loads(raw)
